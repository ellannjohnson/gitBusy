...
import { APP_BUILD, APP_VERSION } from '../appMeta'
...
const primaryItems: Array<{ id: Section; label: string; icon: typeof Library }> = [
  ...
  { id: 'lists', label: 'GitHub lists', icon: List },
  ...
  return (
    <aside className={`sidebar ${sidebarOpen ? 'sidebar--open' : ''}`} aria-label="Primary navigation">
      ...
      <div className="sidebar__account">
        <span className="avatar" aria-hidden="true">{accountLogin ? accountLogin.slice(0, 2).toUpperCase() : '—'}</span>
        <div>
          <strong>{accountLogin || 'Not connected'}</strong>
      ...
      <div className="sidebar__footer">
        ...
        <div className="sidebar__footer-row"><span>v{APP_VERSION} · build {APP_BUILD}</span><GitFork size={14} /></div>
      </div>
    </aside>
  )
}
