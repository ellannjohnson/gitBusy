...
start_server() {
  ...
  if [[ -f "$PID_FILE" ]] && ! pid_file_is_alive_and_ours "$PID_FILE" "$PROJECT"; then
    local stored
  ...
  if [[ ! -x "$NODE" || ! -f "$PRODUCTION_SERVER_ENTRY" ]]; then
    notify 'gitBusy bundle is incomplete — reinstall it'
    return 1
  fi
  local github_client_id="${GITBUSY_GITHUB_CLIENT_ID:-}"
  if [[ -z "$github_client_id" && -r "$BUNDLE_ROOT/gitbusy-github-client-id" ]]; then
    IFS= read -r github_client_id < "$BUNDLE_ROOT/gitbusy-github-client-id" || true
  fi

  (
    cd "$PROJECT"
    GITBUSY_GITHUB_CLIENT_ID="$github_client_id" exec "$NODE" "$PRODUCTION_SERVER_ENTRY" --host 127.0.0.1 --port "$PORT"
  ) > "$LOG_FILE" 2>&1 &
  local pid=$!
  printf '%s\n' "$pid" > "$PID_FILE"
  for _ in {1..40}; do
